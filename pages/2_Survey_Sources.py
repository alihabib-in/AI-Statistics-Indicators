...
for _, row in primary.iterrows():
    ...
    with st.expander(f"{ac.country_flag(row['country_code'])} {row['country']} — {row['survey_name']}"):
...
for _, r in secondary.iterrows():
    st.markdown(f"- **{r['Country']}** — {r['Source']} ({r['Metric']}) [open ↗]({r['Link']})")
